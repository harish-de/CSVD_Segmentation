...
import torch
import unet_model
import cross_validation_sing_ge3t
...
def save_checkpoint(state, count, filename='checkpoints\\checkpoint.pth.tar'):
    ...
    :return:
    """
    filename = 'checkpoints_utr_ge3t\\checkpoint_' + str(count) + '.pth.tar'
...
def validation(val_loader, epoch_index):
    path = 'checkpoints_utr_ge3t\\checkpoint_' + str(epoch_index) + '.pth.tar'
...
train_loader, val_loader = cross_validation_sing_ge3t.create_train_val_dset()
...
with open('plot_loss_sing_ge3t.txt', 'w') as f:
