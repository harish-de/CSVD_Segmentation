...
def create_dset_screening_stage1():
    ...
    dset_train = joblib.load('balanced_dataset_16_16_10.sav')
...
def save_checkpoint(state, count, filename='checkpoints\\checkpoint.pth.tar'):
    ...
    :return:
    """
    filename = 'checkpoints_discrimination_stage1\\checkpoint_' + str(count) + '.pth.tar'
...
def validate(val_loader, epoch_index):
    path = 'checkpoints_discrimination_stage1\\checkpoint_' + str(epoch_index) + '.pth.tar'
...
model = model_discrimination.classifier().cuda()
...
with open('discrimination_stage1_plot_loss.txt', 'w') as f:
