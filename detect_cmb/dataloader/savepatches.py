...
img_dir = 'E:\\abhivanth\\cmb'

out_dir = 'C:\\Users\\bhuva\\PycharmProjects\\detect_cmb\\patches\\output_patches'
# x_shape = 20
# y_shape = 20
# z_shape = 16
...
for file in image_list:
    current_path = os.path.join(img_dir,file)
    file_list = os.listdir(current_path)
    file_list.sort(reverse=True)
    for current_file in file_list:

        if(".nii" not in current_file):
           out_dir = 'C:\\Users\\bhuva\\PycharmProjects\\detect_cmb\\patches\\output_patches'
           print(current_file)

           gt_path = os.path.join(current_path,current_file)

           gt_list = os.listdir(gt_path)



           for gt_images in gt_list:
               base,ext = os.path.splitext(gt_images)
               base,ext= base.split('_gt')
               groundtruth_list.append(base)
               fn = os.path.join(gt_path,gt_images)
               cutpatches(fn)
        else:
            file_name, ext = current_file.split('.')
            # print(current_file)

            if file_name in groundtruth_list:
                fn = os.path.join(current_path, current_file)
                out_dir = 'C:\\Users\\bhuva\\PycharmProjects\\detect_cmb\\patches\\inputpatches'
                cutpatches(fn)
