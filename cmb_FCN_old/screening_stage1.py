...
def create_dset_screening_stage1_train():
    ...
    dset_train = joblib.load('balanced_dataset_train.sav')


    # train_ds, val_ds = trainTestSplit(dset_train,validationRatio)

    ## USE THESE FOR TRAINING & EVALUATING MODEL
    train_loader = torch.utils.data.DataLoader(dset_train, batch_size=batch_size, shuffle=True, num_workers=0)
    ...
    return train_loader
def create_dset_screening_stage1_validation():

    dset_val = joblib.load('balanced_dataset_validation.sav')

    batch_size = 128

    val_loader = torch.utils.data.DataLoader(dset_val, batch_size=batch_size, shuffle=True, num_workers=0)
...
train_loader    = create_dset_screening_stage1_train()

val_loader  = create_dset_screening_stage1_validation()
